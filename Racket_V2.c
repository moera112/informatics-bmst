...
//---------------------------------------------------
// Модуль реализовывает корректное считывание карты
// размером 25/65 для  игры ракетка из файла
//---------------------------------------------------
...
void readMap(FILE* stream){ // Функция читает карту из файла stream в массив ar
    for(int i = 0; i < h; i++){
...
void GameStream(){ // ВКлючает в себя основные функции  нициализации и отображения
    setcur(0, 0);
    init(saveP);
    PutRacket();
    putBall();
...
int main(){
    bool run = false;
    char *str = "maps/player_anim.txt";
    FILE* fp = fopen(str, "r");
    if(fp == NULL){
        perror("player_anim.txt");
        return 1;
    }
